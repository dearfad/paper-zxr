...
3. 预处理流程 (ImagePreprocessor)
   处理步骤：
   中心裁剪（0.8比例）- 聚焦培养核心区
   尺寸统一（336×336px）- 适配CLIP/ViT输入
   CLAHE增强 - clipLimit=2.0, tileGridSize=(8,8)
   去噪 - 高斯模糊(σ=0.5) + 中值滤波(3×3)
...
⚠️ 关键设计要点
双细胞系适配：MDA-MB-231因侵袭性生长边缘不规则，自动降低清晰度阈值至45（MCF-7为55）
时序对齐：预处理包含刚性对齐功能，确保同一孔不同时间点视野一致
命名解析：自动解析 MCF7_001_24h_F01.tif → cell_line/sample_id/timepoint/view_id
质控日志：所有操作记录到CSV，支持后续数据清洗报告和论文方法学描述
无损格式：强制使用TIFF格式，避免JPG压缩伪影
